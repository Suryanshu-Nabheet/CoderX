  ...
  const renderColorSection = () => (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-coderx-elements-textPrimary flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-coderx-elements-item-contentAccent"></div>
          Color Palette
        </h3>
        <button
          onClick={handleReset}
          className="text-sm bg-transparent hover:bg-coderx-elements-bg-depth-2 text-coderx-elements-textSecondary hover:text-coderx-elements-textPrimary rounded-lg flex items-center gap-2 transition-all duration-200"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {paletteRoles.map((role) => (
          <div
            key={role.key}
            className="group flex items-center gap-4 p-4 rounded-xl bg-coderx-elements-bg-depth-3 hover:bg-coderx-elements-bg-depth-2 border border-transparent hover:border-coderx-elements-borderColor transition-all duration-200"
          >
            <div className="relative flex-shrink-0">
              <div
                className="w-12 h-12 rounded-xl shadow-md cursor-pointer transition-all duration-200 hover:scale-110 ring-2 ring-transparent hover:ring-coderx-elements-borderColorActive"
              ...
              <input
                ...
                tabIndex={-1}
              />
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-coderx-elements-bg-depth-1 rounded-full flex items-center justify-center shadow-sm">
                <span className="i-ph:pencil-simple text-xs text-coderx-elements-textSecondary" />
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-coderx-elements-textPrimary transition-colors">{role.label}</div>
              <div className="text-sm text-coderx-elements-textSecondary line-clamp-2 leading-relaxed">
                {role.description}
              </div>
              <div className="text-xs text-coderx-elements-textTertiary font-mono mt-1 px-2 py-1 bg-coderx-elements-bg-depth-1 rounded-md inline-block">
                {palette[role.key]}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {designFonts.map((f) => (
          <button
            ...
            className={`group p-4 rounded-xl border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-coderx-elements-borderColorActive ${
              font.includes(f.key)
                ? 'bg-coderx-elements-item-backgroundAccent border-coderx-elements-borderColorActive shadow-lg'
                : 'bg-coderx-elements-background-depth-3 border-coderx-elements-borderColor hover:border-coderx-elements-borderColorActive hover:bg-coderx-elements-bg-depth-2'
            }`}
          >
            <div className="text-center space-y-2">
              <div
                className={`text-2xl font-medium transition-colors ${
                  font.includes(f.key) ? 'text-coderx-elements-item-contentAccent' : 'text-coderx-elements-textPrimary'
                }`}
              ...
              {font.includes(f.key) && (
                <div className="w-6 h-6 mx-auto bg-coderx-elements-item-contentAccent rounded-full flex items-center justify-center">
          ...
          return (
            <div key={f.key} className="feature-card-container p-2">
              <button
                ...
                className={`group relative w-full p-6 text-sm font-medium transition-all duration-200 bg-coderx-elements-background-depth-3 text-coderx-elements-item-textSecondary ${
                  f.key === 'rounded'
                    ? isSelected
                      ? 'rounded-3xl'
                      : 'rounded-xl'
                  ...
                  f.key === 'border'
                    ? isSelected
                      ? 'border-3 border-coderx-elements-borderColorActive bg-coderx-elements-item-backgroundAccent text-coderx-elements-item-contentAccent'
                      : 'border-2 border-coderx-elements-borderColor hover:border-coderx-elements-borderColorActive text-coderx-elements-textSecondary'
                    : f.key === 'gradient'
                      ? ''
                      : isSelected
                        ? 'bg-coderx-elements-item-backgroundAccent text-coderx-elements-item-contentAccent shadow-lg'
                        : 'bg-coderx-elements-bg-depth-3 hover:bg-coderx-elements-bg-depth-2 text-coderx-elements-textSecondary hover:text-coderx-elements-textPrimary'
                } ${f.key === 'shadow' ? (isSelected ? 'shadow-xl' : 'shadow-lg') : 'shadow-md'}`}
                style={{
                  ...(f.key === 'gradient' && {
                    background: isSelected
                      ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'
                      : 'var(--coderx-elements-bg-depth-3)',
                    color: isSelected ? 'white' : 'var(--coderx-elements-textSecondary)',
                  }),
                }}
              >
                <div className="flex flex-col items-center gap-4">
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-coderx-elements-bg-depth-1 bg-opacity-20">
                    {f.key === 'rounded' && (
                      <div
    ...
    <div>
      ...
      <DialogRoot open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <Dialog showCloseButton={false}>
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            <div className="">
              <DialogTitle className="text-2xl font-bold text-coderx-elements-textPrimary">
                Design Palette & Features
              </DialogTitle>
              <DialogDescription className="text-coderx-elements-textSecondary leading-relaxed">
            ...
            {/* Navigation Tabs */}
            <div className="flex gap-1 p-1 bg-coderx-elements-bg-depth-3 rounded-xl">
              {[
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-all duration-200 ${
                    activeSection === tab.key
                      ? 'bg-coderx-elements-background-depth-3 text-coderx-elements-textPrimary shadow-md'
                      : 'bg-coderx-elements-background-depth-2 text-coderx-elements-textSecondary hover:text-coderx-elements-textPrimary hover:bg-coderx-elements-bg-depth-2'
                  }`}
                >
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-3">
                ...
                <Button
                  variant="ghost"
                  onClick={handleSave}
                  className="bg-coderx-elements-button-primary-background hover:bg-coderx-elements-button-primary-backgroundHover text-coderx-elements-button-primary-text"
                >
                  Save Changes
                </Button>
              </div>
      ...
      <style>{`
        .custom-scrollbar {
          ...
          scrollbar-color: var(--coderx-elements-textTertiary) transparent;
        }
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background-color: var(--coderx-elements-textTertiary);
          border-radius: 3px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background-color: var(--coderx-elements-textSecondary);
        }
        .line-clamp-2 {
          display: -webkit-box;
          -webkit-line-clamp: 2;
