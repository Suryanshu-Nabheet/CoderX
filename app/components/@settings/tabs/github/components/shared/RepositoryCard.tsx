...
export function RepositoryCard({
      ...
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-medium text-coderx-elements-textPrimary">{repository.name}</h4>
            {repository.private && <Lock className="w-3 h-3 text-coderx-elements-textTertiary" />}
            {repository.fork && <GitFork className="w-3 h-3 text-coderx-elements-textTertiary" />}
            {repository.archived && <Archive className="w-3 h-3 text-coderx-elements-textTertiary" />}
          </div>
        ...
        {repository.description && (
          <p className="text-xs text-coderx-elements-textSecondary mb-2 line-clamp-2">{repository.description}</p>
        )}

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-coderx-elements-textTertiary">
            {repository.language && (
              <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1 text-xs text-coderx-elements-textTertiary">
  ...
  return (
    <Component
      {...interactiveProps}
      className={classNames(
        'block p-4 rounded-lg bg-coderx-elements-background-depth-1 dark:bg-coderx-elements-background-depth-1 border border-coderx-elements-borderColor dark:border-coderx-elements-borderColor relative',
    ...
    >
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <GitBranch className="w-4 h-4 text-coderx-elements-icon-tertiary" />
            <h5
              className={classNames(
            ...
            {repository.fork && (
              <span title="Forked repository">
                <GitFork className="w-3 h-3 text-coderx-elements-textTertiary" />
              </span>
            )}
            {repository.archived && (
              <span title="Archived repository">
                <Archive className="w-3 h-3 text-coderx-elements-textTertiary" />
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 text-xs text-coderx-elements-textSecondary">
            <span className="flex items-center gap-1" title="Stars">
              <Star className="w-3.5 h-3.5 text-coderx-elements-icon-warning" />
              {repository.stargazers_count.toLocaleString()}
            </span>
            <span className="flex items-center gap-1" title="Forks">
              <GitFork className="w-3.5 h-3.5 text-coderx-elements-icon-info" />
              {repository.forks_count.toLocaleString()}
            </span>
            {showExtendedMetrics && repository.issues_count !== undefined && (
              <span className="flex items-center gap-1" title="Open Issues">
                <Circle className="w-3.5 h-3.5 text-coderx-elements-icon-error" />
            ...
            {showExtendedMetrics && repository.pull_requests_count !== undefined && (
              <span className="flex items-center gap-1" title="Pull Requests">
                <GitPullRequest className="w-3.5 h-3.5 text-coderx-elements-icon-success" />
        ...
        <div className="space-y-2">
          {repository.description && (
            <p className="text-xs text-coderx-elements-textSecondary line-clamp-2">{repository.description}</p>
          )}

          {/* Repository metrics bar */}
          <div className="flex items-center gap-2 text-xs">
            {repository.license && (
              <span className="px-2 py-0.5 rounded-full bg-coderx-elements-background-depth-2 text-coderx-elements-textTertiary">
