  ...
  return (
    <div
      className={classNames(
        'flex flex-col items-center justify-center py-8 text-coderx-elements-textSecondary',
        className,
      )}
    >
      <Loader2 className={classNames('animate-spin mb-2', sizeClasses[size])} />
      <p className={classNames('text-coderx-elements-textSecondary', textSizeClasses[size])}>{message}</p>
    </div>
  );
}
...
export function ErrorState({
  ...
  return (
    <div className={classNames('flex flex-col items-center justify-center py-8 text-center', className)}>
      ...
      <h3 className={classNames('font-medium text-coderx-elements-textPrimary mb-1', textSizeClasses[size])}>{title}</h3>
      <p className={classNames('text-coderx-elements-textSecondary mb-4', textSizeClasses[size])}>{message}</p>
      {onRetry && (
        <button
          onClick={onRetry}
...
export function GitHubConnectionRequired({ onConnect, className = '' }: GitHubConnectionRequiredProps) {
  return (
    <div className={classNames('flex flex-col items-center justify-center py-12 text-center', className)}>
      <Github className="w-12 h-12 text-coderx-elements-textTertiary mb-4" />
      <h3 className="text-lg font-medium text-coderx-elements-textPrimary mb-2">GitHub Connection Required</h3>
      <p className="text-sm text-coderx-elements-textSecondary mb-6 max-w-md">
