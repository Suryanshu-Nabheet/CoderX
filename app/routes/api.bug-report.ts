...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  body += '---\n*Submitted via CoderX bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'Suryanshu-Nabheet/CoderX';
