...
type UnistNode = { type: string; position?: any; [key: string]: any };
type UnistParent = { children: any[]; [key: string]: any } | undefined | null;
  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__coderxThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__coderxArtifact__', '__coderxThought__', '__coderxQuickAction', '__coderxSelectedElement__'],

      // ['className', '__coderxThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__coderxArtifact__', '__coderxThought__', '__coderxQuickAction'],
    ],
  },
  strip: [],
};
