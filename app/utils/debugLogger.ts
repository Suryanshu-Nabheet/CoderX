...
class DebugLogger {
  ...
  private _getCurrentModel(): string {
    try {
      ...
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('coderx_current_model');
  ...
  private _getCurrentProvider(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('coderx_current_provider');
  ...
  private _getProjectType(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('coderx_project_type');
  ...
  private _getGitInfoFallback(): AppInfo['gitInfo'] {
    try {
      ...
      const stored = localStorage.getItem('coderx_git_info');
