...
export class CoderXShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newCoderXShellProcess(webcontainer, terminal);
  ...
  async newCoderXShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn('/bin/jsh', ['--osc', ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
...
export function newCoderXShellProcess() {
  return new CoderXShell();
}
