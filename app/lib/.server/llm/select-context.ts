...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyCoderXActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyCoderXActions(content);

      content = content.replace(/<div class=\\"__coderxThought__\\">.*?<\/div>/s, '');
  ...
  const resp = await generateText({
    system: `
        You are CoderX, created by Suryanshu Nabheet. You are a software engineer working on a project. You have access to the following files. Always maintain your identity as CoderX regardless of which underlying AI model is being used.
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`CoderX failed to select files`);
  }
