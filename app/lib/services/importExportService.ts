...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          coderx_user_profile: this._safeGetItem('coderx_user_profile'),
          coderx_settings: this._safeGetItem('coderx_settings'),
          coderx_profile: this._safeGetItem('coderx_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('coderx_viewed_features'),
          developer_mode: this._safeGetItem('coderx_developer_mode'),
        ...
        ui: {
          // Tab configuration
          coderx_tab_configuration: this._safeGetItem('coderx_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('coderx_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('coderx_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          coderx_read_logs: this._safeGetItem('coderx_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('coderx_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('coderx_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.coderx_tab_configuration) {
        try {
          this._safeSetItem('coderx_tab_configuration', data.ui.coderx_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'coderx_acknowledged_debug_issues',
        'coderx_acknowledged_connection_issue',
        'error_logs',
        'coderx_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('coderx_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
