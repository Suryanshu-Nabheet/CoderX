...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(CODERX_QUICK_ACTIONS_OPEN, i)) {
        const actionsBlockEnd = input.indexOf(CODERX_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + CODERX_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <coderx-quick-action ...>label</coderx-quick-action> inside
          const quickActionRegex = /<coderx-quick-action([^>]*)>([\s\S]*?)<\/coderx-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + CODERX_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
