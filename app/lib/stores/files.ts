...
export class FilesStore {
  ...
  constructor(webcontainerPromise: Promise<WebContainer>) {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        const deletedPathsJson = localStorage.getItem('coderx-deleted-paths');
  ...
  #persistDeletedPaths() {
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem('coderx-deleted-paths', JSON.stringify([...this.#deletedPaths]));
      }
    } catch (error) {
...
function isBinaryFile(buffer: Uint8Array | undefined) {
  ...
  // Keep detection browser-native so the client bundle does not pull in a
  // Node-oriented binary inspection package.
